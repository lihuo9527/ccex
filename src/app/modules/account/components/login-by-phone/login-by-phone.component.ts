...
@Component({
  selector: 'ccex-login-by-phone',
  templateUrl: './login-by-phone.component.html',
  styleUrls: ['./login-by-phone.component.less']
})
export class LoginByPhoneComponent implements OnInit {
  ...
  private login(validGt) {
    this.apiClient.login(
      this.validateForm.controls.countryCode.value + this.validateForm.controls.phone.value,
  ...
  async onGoogleValidate(event: any) {
    ...
    try {
      await this.apiClient.login_ga(
              this.validateForm.controls.countryCode.value + this.validateForm.controls.phone.value,
              this.validateForm.controls.password.value, event.code
            );
      this.isOver = true;
      window.location.href = this.returnUrl ? this.returnUrl : '/wallet';
    } catch (e) {
